...
3. Visiting `/notify` with no slug shows the same form plus a radio list of _all_ courses in `app/config/courses.ts`; the course is required, but only by a client-side check in `NotifyForm.tsx`.
...
6. If a bin-full report for that course was already submitted from this browser inside the rate-limit window, the _loader_ already renders the thank-you screen — the form is never shown, and a POST is silently discarded (still answered with success).
...
- **Rate limit (bin-full only)**, `app/features/notifications/binFullRateLimit.server.ts`:
  ...
  - Every POST rewrites the cookie, so repeated clicks _restart_ the window rather than expire it.
