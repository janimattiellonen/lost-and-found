...
- Every write scopes on the club so that an id belonging to the other club cannot be acted on: disc create, delete, batch delete, the mark-\* updates, retrieval errands (`queryDiscIdByExternalId.server.ts` and its plural, `queryDiscIdsByExternalIds.server.ts`), and owner-response handling all add `.eq('club_id', currentClubId())` or pass `p_club_id`.
...
- A visitor on either deployment can open `/notify/tali` or `/bin/full/oittaa` regardless of which club that course belongs to; the report is then filed under the _instance's_ `APP_CLUB_ID` while naming the other club's course.
